...
class HomeController extends Controller
{
  ...
  public function post_register(Request $request)
  {
    ...
    if ($customer = Customer::create($data)) {
      Mail::send('emails.active_account', compact('customer'), function ($email) use ($customer) {
        $email->subject('Lamy Store - Xác nhận tài khoản');
  ...
  public function post_forget(Request $request)
  {
    ...
    Mail::send('emails.forget_account', compact('customer'), function ($email) use ($customer) {
      $email->subject('Lamy Store - Lấy lại mật khẩu tài khoản');
      $email->to($customer->email, $customer->name);
    });
  ...
  {
    $name = 'Phan Trần Thu Phương';
    Mail::send(
      'emails.test',
      compact('name'),
      function ($email) use ($name) {
        $email->subject('Đơn đặt hàng');
        $email->to('phantranthuphuong00@gmail.com', $name);
      }
    );
  }
}
